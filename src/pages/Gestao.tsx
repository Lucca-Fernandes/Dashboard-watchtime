...
// ... (nenhuma mudança nos types e na função checkUserEmailDomain)
...
const TotalStudentsModuleChart = ({
  data,
  onBarClick,
  selectedDomainFilter
}: {
  data: ModuleCompletionData[],
  onBarClick: (module: string, students: Set<string>) => void,
  selectedDomainFilter: 'all' | 'pditabira' | 'pdbomdespacho'
}) => {
  const { data: allWatchTimeData } = useData();
  ...
  const studentsByModule = useMemo(() => {
    ...
    const filteredData = allWatchTimeData.filter(item => {
      ...
      return (status === 'Ativo' || status === 'EmRecuperacao' || status === 'Atencao') && checkUserEmailDomain(item.user_email, selectedDomainFilter);
    });
    filteredData.forEach(item => {
      ...
      for (const [moduleName, moduleDisciplines] of Object.entries(modules)) {
        if (moduleDisciplines.includes(normalizedCourseName)) {
          if (!studentsMap.has(moduleName)) {
            studentsMap.set(moduleName, new Set<string>());
          }
          studentsMap.get(moduleName)!.add(item.user_email);
          break;
        }
      }
    });
    return studentsMap;
  }, [allWatchTimeData, modules, courseNameMappings, selectedDomainFilter]);
  ...
  return (
    <Box sx={{ mt: 4, mb: 4 }}>
      ...
      <ResponsiveContainer width="100%" height={300}>
        {/* CORREÇÃO FINAL: Usar onClick no BarChart com 'activeLabel' */}
        <BarChart
          data={data}
          margin={{ top: 40, right: 30, left: 20, bottom: 5 }}
          onClick={(state: any) => {
            // 'activeLabel' é a propriedade confiável que contém o nome do módulo no eixo X
            if (state && state.activeLabel) {
              const moduleName = state.activeLabel;
              const students = studentsByModule.get(moduleName) || new Set<string>();
              onBarClick(moduleName, students);
            }
          }}
...
const ActiveStudentsModuleChart = ({
  data,
  onBarClick,
  unifiedDisciplineCompletion,
  modules,
  selectedDomainFilter
}: {
  data: ModuleCompletionData[],
  onBarClick: (module: string, students: Set<string>) => void,
  unifiedDisciplineCompletion: Map<string, Set<string>>,
  modules: { [key: string]: string[] },
  selectedDomainFilter: 'all' | 'pditabira' | 'pdbomdespacho'
}) => {
  const { data: allWatchTimeData } = useData();

  const activeStudentsByModule = useMemo(() => {
    const studentsWithActivityInModule = new Map<string, Set<string>>();
    const allActiveStudents = new Set<string>();
    if (!allWatchTimeData || allWatchTimeData.length === 0 || !unifiedDisciplineCompletion) {
    ...
    filteredData.forEach(item => {
      ...
      for (const [moduleName, moduleDisciplines] of Object.entries(modules)) {
        if (moduleDisciplines.includes(normalizedCourseName)) {
          if (!studentsWithActivityInModule.has(moduleName)) {
            studentsWithActivityInModule.set(moduleName, new Set<string>());
          }
          studentsWithActivityInModule.get(moduleName)!.add(item.user_email);
          break;
        }
      }
    });
    ...
    const finalModuleStudents: { [module: string]: Set<string> } = {};
    moduleOrder.forEach(m => finalModuleStudents[m] = new Set());
    for (const studentEmail of allActiveStudents) {
      let completedAllPrevious = true;
      for (const moduleName of moduleOrder) {
        const disciplinesInModule = modules[moduleName];
        ...
        if (hasCompletedCurrentModule) {
          ...
          finalModuleStudents[moduleName].add(studentEmail);
          break;
        }
      }
    }
    return new Map(Object.entries(finalModuleStudents));
  }, [allWatchTimeData, modules, courseNameMappings, selectedDomainFilter, unifiedDisciplineCompletion]);
      ...
      <ResponsiveContainer width="100%" height={300}>
        ...
        <BarChart
          ...
          onClick={(state: any) => {
            ...
            if (state && state.activeLabel) {
              const moduleName = state.activeLabel;
              const students = activeStudentsByModule.get(moduleName) || new Set<string>();
              onBarClick(moduleName, students);
            }
          }}
          style={{ cursor: 'pointer' }}
...
// ... (o restante do arquivo Gestao.tsx permanece exatamente o mesmo, sem alterações)
const DisciplineModuleChart = ({ 
  data, 
  moduleName, 
  onBarClick, 
  ...
  onBarClick: (discipline: string, students: Set<string>) => void; 
  studentsByDiscipline: { [key: string]: Set<string> }; 
}) => {
      ...
      <Box sx={{ mt: 4, mb: 4 }}>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <BarChart 
            ...
            onClick={(state: any) => {
              if (state && state.activeLabel) {
                const discipline = state.activeLabel;
                const students = studentsByDiscipline[discipline] || new Set<string>();
                onBarClick(discipline, students);
              }
            }}
            style={{ cursor: 'pointer' }}
          >
  ...
  const DisciplineGradeModuleChart = ({ 
    data, 
    moduleName, 
    onBarClick, 
    studentsByDiscipline 
  ...
  const Gestao: React.FC = () => {
    ...
    const [openModuleStudents, setOpenModuleStudents] = useState<{ module: string; students: Set<string> } | null>(null);
    const [openDisciplineStudentsModule, setOpenDisciplineStudentsModule] = useState<{ discipline: string; students: Set<string>; type: 'aulas' | 'notas' } | null>(null);
    const navigate = useNavigate();
    const [gradesDataMap, setGradesDataMap] = useState<{ [discipline: string]: GradeData[] }>({});
  
    useEffect(() => {
      console.log('allWatchTimeData:', allWatchTimeData);
    }, [allWatchTimeData]);
    ...
    const agents = useMemo(() => {
      ...
      const uniqueAgents = [...new Set(allWatchTimeData.map(item => item.ags).filter(ags => ags && typeof ags === 'string'))];
    ...
    const preProcessedAgentCompletionData: PreProcessedAgentData | null = useMemo(() => {
      if (!isDataLoaded || !allWatchTimeData || allWatchTimeData.length === 0 || agents.length === 0) {
        console.log('Dados insuficientes para preProcessedAgentCompletionData:', { isDataLoaded, allWatchTimeData, agents });
        return null;
      }
      const filteredData = allWatchTimeData.filter(item => {
        const status = item.status?.trim();
      ...
      filteredData.forEach(item => {
        if (item.ags && typeof item.ags === 'string' && agents.includes(item.ags) && checkUserEmailDomain(item.user_email, selectedDomainFilter)) {
        ...
        disciplines.forEach((discipline: string) => {
          ...
          if (totalLessons > 0) {
            ...
            disciplineSpecificAgentData.forEach(item => {
              ...
              if (isLessonCompletedByDuration && item.user_email) {
            ...
            const completedStudents = Object.keys(studentProgress).filter(email => {
    ...
    const dashboardData = useMemo(() => {
      ...
      const newDashboardData = agents.map(agent => {
        ...
        return { agent: agent.split('@')[0], concludedStudents: totalConcluded };
      });
    ...
    const filteredUsernames = useMemo(() => {
      ...
      if (!allWatchTimeData) return usernames;
      allWatchTimeData.forEach(item => {
        if (checkUserEmailDomain(item.user_email, selectedDomainFilter)) {
          const username = item.user_email.split('@')[0]?.toLowerCase().trim();
    ...
    const allDisciplineCompletionData = useMemo(() => {
      if (!isDataLoaded || !allWatchTimeData || allWatchTimeData.length === 0) {
        console.log('Dados insuficientes para allDisciplineCompletionData:', { isDataLoaded, allWatchTimeData });
        return null;
      }
      const filteredData = allWatchTimeData.filter(item => {
        const status = item.status?.trim();
      ...
      filteredData.forEach(_item => {
        if (checkUserEmailDomain(_item.user_email, selectedDomainFilter)) {
          ...
          const _isLessonCompletedByDuration = _videoTotalSeconds > 0 && _totalSecondsWatched >= (_videoTotalSeconds * 0.5);
    ...
    const allDisciplineGradeCompletionData = useMemo(() => {
      ...
      Object.keys(disciplineCsvMap).forEach(discipline => {
        ...
        disciplineData.forEach(item => {
          const username = item['Usuário']?.split('@')[0]?.trim().toLowerCase();
          const nota = item['Nota total (%)']?.trim().replace(',', '.');
          if (username && nota && !isNaN(parseFloat(nota)) && filteredUsernames.has(username)) {
            const grade = parseFloat(nota);
            if (grade >= 60) {
              approvedStudents.add(username);
            }
          }
        });
    ...
    const totalStudentsModuleData = useMemo(() => {
      if (!isDataLoaded || !allWatchTimeData || allWatchTimeData.length === 0) {
        console.log('Dados insuficientes para totalStudentsModuleData:', { isDataLoaded, allWatchTimeData });
        return [];
      }
    ...
    const allDisciplineGradeCompletionStudents = useMemo(() => {
      ...
      Object.keys(disciplineCsvMap).forEach(discipline => {
        ...
        const approvedStudents = new Set<string>();
        disciplineData.forEach(item => {
          const userEmail = item['Usuário']?.trim().toLowerCase();
          if (!userEmail) return;
          ...
          const nota = item['Nota total (%)']?.trim().replace(',', '.');
          if (fullEmail && nota && !isNaN(parseFloat(nota))) {
            if (parseFloat(nota) >= 60) {
              approvedStudents.add(fullEmail);
            }
          }
        });
    ...
    const activeStudentsModuleData = useMemo(() => {
      if (!isDataLoaded || !allWatchTimeData || allWatchTimeData.length === 0 || !unifiedDisciplineCompletion) {
        console.log('Dados insuficientes para activeStudentsModuleData:', { isDataLoaded, allWatchTimeData, unifiedDisciplineCompletion });
        return [];
      }
      const studentsWithActivityInModule = new Map<string, Set<string>>();
      const allActiveStudents = new Set<string>();
      ...
      const chartData: ModuleCompletionData[] = moduleOrder.map(moduleName => ({
        module: moduleName,
        totalStudents: finalModuleCounts[moduleName] || 0
      }));
      return chartData;
    ...
    const filteredDisciplineChartData = useMemo(() => {
      ...
      disciplinesInModule.forEach((discipline: string) => {
          const completionDetail = allDisciplineCompletionData[discipline];
          const concludedCount = completionDetail ? completionDetail.count : 0;
          chartData.push({ discipline: discipline, concludedStudents: concludedCount });
      });
    ...
    const filteredDisciplineGradeChartData = useMemo(() => {
      ...
      const chartData: DisciplineChartData[] = [];
      disciplinesInModule.forEach((discipline: string) => {
        const concludedCount = allDisciplineGradeCompletionData[discipline]?.count || 0;
        chartData.push({ discipline: discipline, concludedStudents: concludedCount });
      });
      chartData.sort((a, b) => a.discipline.localeCompare(b.discipline));
      return chartData;
    ...
    const handleModuleClick = (module: string, students: Set<string>) => {
      setOpenModuleStudents({ module, students });
    };
    ...
    return (
      <Box sx={{ p: 4 }}>
        ...
        <TotalStudentsModuleChart
          data={totalStudentsModuleData}
          onBarClick={handleModuleClick}
          selectedDomainFilter={selectedDomainFilter}
        />
        <ActiveStudentsModuleChart
          data={activeStudentsModuleData}
          onBarClick={handleModuleClick}
          unifiedDisciplineCompletion={unifiedDisciplineCompletion}
          modules={modules}
          selectedDomainFilter={selectedDomainFilter}
        />
        ...
        <Paper sx={{ p: 3, mt: 4, mb: 4 }}>
          ...
          <DisciplineModuleChart 
            data={filteredDisciplineChartData} 
            moduleName={selectedModuleForChart} 
            onBarClick={(discipline: string, students: Set<string>) => setOpenDisciplineStudentsModule({ discipline, students, type: 'aulas' })}
            studentsByDiscipline={Object.fromEntries(Object.entries(allDisciplineCompletionData).map(([disc, detail]) => [disc, detail.students]))}
          />
          <DisciplineGradeModuleChart 
            data={filteredDisciplineGradeChartData} 
            moduleName={selectedModuleForChart} 
            onBarClick={(discipline: string, students: Set<string>) => setOpenDisciplineStudentsModule({ discipline, students, type: 'notas' })}
            studentsByDiscipline={Object.fromEntries(allDisciplineGradeCompletionStudents)}
          />
        </Paper>
        ...
        {openModuleStudents && (
          <Dialog open onClose={() => setOpenModuleStudents(null)} maxWidth="sm" fullWidth>
            <DialogTitle>Alunos em "{openModuleStudents.module}"</DialogTitle>
            <DialogContent dividers>
              <List dense>
                {openModuleStudents.students.size > 0 ? (
                  Array.from(openModuleStudents.students).map((studentEmail, index) => (
                    <ListItem key={index} sx={{ py: 0.5 }}>
                      <ListItemText primary={studentEmail} />
                    </ListItem>
                  ))
                ) : (
                  <Typography sx={{ p: 2 }}>Nenhum aluno encontrado para este módulo.</Typography>
                )}
              </List>
            </DialogContent>
          </Dialog>
      ...
      {openDisciplineStudentsModule && (
        <Dialog open onClose={() => setOpenDisciplineStudentsModule(null)} maxWidth="sm" fullWidth>
          <DialogTitle>Alunos Concluídos em "{openDisciplineStudentsModule.discipline}" (Baseado em {openDisciplineStudentsModule.type === 'aulas' ? 'Aulas' : 'Notas'})</DialogTitle>
          <DialogContent dividers>
            <List dense>
              {openDisciplineStudentsModule.students.size > 0 ? (
                Array.from(openDisciplineStudentsModule.students).map((studentEmail, index) => (
                  <ListItem key={index} sx={{ py: 0.5 }}>
                    <ListItemText primary={studentEmail} />
                  </ListItem>
                ))
