...
const DisciplineGradeModuleChart = ({ data, moduleName }: { data: DisciplineChartData[]; moduleName: string | null }) => {
  if (!moduleName || data.length === 0) {
    return <Typography variant="h6">Selecione um módulo para ver os detalhes das disciplinas baseados em notas.</Typography>;
  }
...
const Gestao: React.FC = () => {
  ...
  useEffect(() => {
    const loadAllGrades = async () => {
      if (Object.keys(gradesDataMap).length > 0) {
        console.log('gradesDataMap already loaded, skipping fetch');
        return;
      }
      ...
      for (const [discipline, csvPath] of Object.entries(disciplineCsvMap)) {
        try {
          console.log(`Fetching CSV for ${discipline} at ${csvPath}`);
          const response = await fetch(csvPath);
          if (!response.ok) {
            throw new Error(`Failed to fetch ${csvPath}: ${response.statusText}`);
          }
          const csvText = await response.text();
          console.log(`Raw CSV for ${discipline}:`, csvText.substring(0, 200));
          ...
          const validData = results.data.filter(
            (item) => item && item['Usuário'] !== undefined && item['Nota total (%)'] !== undefined
          );
          loadedData[discipline] = validData;
          console.log(`CSV de ${discipline} carregado com sucesso: ${validData.length} linhas válidas`);
        } catch (error) {
      ...
      console.log('Final gradesDataMap:', loadedData);
    };
    loadAllGrades();
  }, []);
  ...
  const filteredUsernames = useMemo(() => {
    ...
    console.log('filteredUsernames:', Array.from(usernames));
    return usernames;
  ...
  const allDisciplineGradeCompletionData = useMemo(() => {
    ...
    console.log('filteredUsernames:', Array.from(filteredUsernames));
    Object.keys(disciplineCsvMap).forEach(discipline => {
      const disciplineData = gradesDataMap[discipline] || [];
      console.log(`Discipline: ${discipline}, Dados carregados: ${disciplineData.length} linhas`);
      ...
      disciplineData.forEach(item => {
        ...
        if (!username) {
          console.log(`Skipping row in ${discipline}: Missing Usuário`, item);
          return;
        }
        if (!nota) {
          console.log(`Skipping row in ${discipline}: Missing Nota total (%)`, item);
          return;
        }
        if (isNaN(parseFloat(nota))) {
          console.log(`Skipping row in ${discipline}: Invalid Nota total (%) '${nota}'`, item);
          return;
        }
        if (!filteredUsernames.has(username)) {
          console.log(`Skipping row in ${discipline}: Username ${username} not in filteredUsernames`, item);
          return;
        }
      ...
      console.log(`Discipline ${discipline} - Approved Students: ${approvedStudents.size}`);
    });
    return results;
  }, [gradesDataMap, filteredUsernames]);

  const filteredDisciplineGradeChartData = useMemo(() => {
    if (!selectedModuleForChart || !allDisciplineGradeCompletionData) {
      return [];
    }
    ...
    console.log('filteredDisciplineGradeChartData:', chartData);
  ...
  const filteredDisciplineChartData = useMemo(() => {
    ...
    console.log('filteredDisciplineChartData:', chartData);
    chartData.sort((a, b) => a.discipline.localeCompare(b.discipline));
